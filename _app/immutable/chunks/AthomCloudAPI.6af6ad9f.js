...
import { j as client_method, k as derived, w as writable } from "./singletons.e78ae40c.js";
import { p as page } from "./stores.b5dfaa6a.js";
...
let Device$5 = class Device2 extends Item$5 {
  ...
  async getAdvancedFlows() {
    ...
    return Object.values(advancedFlows2).filter((advancedFlow) => {
  ...
  class HomeyAPIV3 extends HomeyAPI {
    constructor({
      ...
      Object.defineProperty(this, "__baseUrlPromise", {
        value: typeof baseUrl2 === "string" ? Promise.resolve(baseUrl2) : null,
        enumerable: false,
        writable: true
      });
    ...
    get baseUrl() {
      return (async () => {
        if (!this.__baseUrlPromise) {
          this.__baseUrlPromise = this.discoverBaseUrl().then(({ baseUrl: baseUrl2 }) => baseUrl2);
    ...
    async discoverBaseUrl() {
      ...
      promise.then(({ baseUrl: baseUrl2, strategyId }) => {
        this.__baseUrl = baseUrl2;
        this.__strategyId = strategyId;
      }).catch(() => {
      });
      const ping = async (strategyId, timeout) => {
        let pingTimeout;
        const baseUrl2 = urls[strategyId];
        return Promise.race([
          Util.fetch(`${baseUrl2}/api/manager/system/ping?id=${this.id}`, {
            headers: {
              "X-Homey-ID": this.id
            }
          }).then(async (res) => {
    ...
    async call({
      ...
      const baseUrl2 = await this.baseUrl;
      ...
      this.__debug(method, `${baseUrl2}${path}`);
      const res = await Util.timeout(Util.fetch(`${baseUrl2}${path}`, {
        method,
        headers,
        body
      }), $timeout);
      ...
      if (!this.io) {
        this.io = Promise.resolve().then(async () => {
          ...
          if (!this.__token)
            await this.login();
          return new Promise((resolve, reject) => {
            this.__debug(`SocketIOClient ${baseUrl2}`);
            this.__io = SocketIOClient(baseUrl2, {
              autoConnect: false,
...
let API$1 = (_h = class {
  constructor({
    ...
    this.baseUrl = baseUrlEnv || baseUrl2 || `https://${this.constructor.SPECIFICATION.host}${this.constructor.SPECIFICATION.basePath}`;
  ...
  */
  ...
  getLoginUrl({
    ...
    if (Array.isArray(scopes2)) {
      search.append("scope", scopes2.join(","));
    }
